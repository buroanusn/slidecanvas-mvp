...
function getWorkspace(projectId?: string): string {
...
function createEmptyProject(title: string): SlideCanvasProject {
  return {
    version: 1,
...
server.registerTool(
  ...
  async ({ title, project_id }) => {
    try {
      ...
      const wsDir = DEFAULT_WORKSPACE;
      if (!existsSync(wsDir)) mkdirSync(wsDir);
    ...
    try {
      ...
      if (!apiKey || apiKey === "your-openai-api-key-here") {
        ...
        for (const t of lines.slice(0, 3)) {
          ...
          if (trimmed.length > 4) bullets.push("- " + trimmed + ": detailed analysis of background and significance");
        }
        if (bullets.length === 0) bullets.push("- " + text.trim() + ": detailed explanation");
...
server.registerTool(
  ...
  async ({ project_id }) => {
    try {
      ...
      let deleted = [];
      if (existsSync(jsonPath)) { await unlink(jsonPath); deleted.push(".json"); }
      if (existsSync(htmlPath)) { await unlink(htmlPath); deleted.push(".html"); }
      if (deleted.length === 0) {
        return { content: [{ type: "text", text: "Project '" + project_id + "' not found." }], isError: true };
      }
...
server.registerResource(
  "project-data",
  new ResourceTemplate("slidecanvas://project/{projectId}", {
    list: async () => {
      const wsDir = DEFAULT_WORKSPACE;
      if (!existsSync(wsDir)) return { resources: [] };
      const files = await readdir(wsDir);
      return {
        resources: files
...
async function loadProject(projectId: string): Promise<SlideCanvasProject> {
  const filePath = join(DEFAULT_WORKSPACE, projectId + ".json");
  if (!existsSync(filePath)) {
...
async function saveProject(projectId: string, project: SlideCanvasProject): Promise<void> {
  const wsDir = DEFAULT_WORKSPACE;
  if (!existsSync(wsDir)) mkdirSync(wsDir);
  project.updatedAt = new Date().toISOString();
...
function countSlides(json: any): number {
...
function extractPlainText(json: any): string {
  ...
  if (json.content) return json.content.map(extractPlainText).join("\n");
  return "";
}
